...
Итого, у нас есть модель, которая общается с контроллером и view. На данный момент, и контроллер, и view реализованы на основе терминала.

![](./screenshots/components.png)

К игре прилагается набор заранее сконструированных карт, игрок проходит их поочередно: при победе на одной карте он может либо перейти на следующую, либо выйти из игры (на самом деле, выйти можно в любой момент)
![](./screenshots/usecases.png)

Инвентарь игрока может пополняться при переходе в клетку, содержающую элемент, который можно взять. Любой из элементов в инвентаре можно в любой момент включить, тогда он пропадает из инвентаря
![](./screenshots/inventory.png)


Персонаж двигается через игровое поле, теряя при этом очки хода. Цель -- добраться до верхнего края карты и перейти через него, не потеряв все очки хода. При столкновении с птицей персонаж теряет 5 очков хода, с грозовым облаком -- два очка
![](./screenshots/gamecases.png)


С точки зрения ООП, архитектура выгдлядит так: есть основной класс Model. Его инстанс содержит в себе:

- игровое поле (матрицу элементов абстрактного типа Cell)
- view-объект для отображения происходящего на экран (в нашем случае -- типа TerminalView)
- controller-объект для получаения команд (в нашем случае -- тоже на основе терминала)
- функцию для загрузки карт из пакета
Класс Cell имеет слудующих наследников (варианты клеток на поле):
- Hero -- персонаж
- Cloud -- облако, при попытке пройти через него ничего не происходит
- ThunderCloud -- облако, через которое тоже нельзя пройти, но при столкновении с ним игрок теряет два очка хода
- Bird -- птица, которая движется сама, при столкновении с ней игрок теряет очки хода
- Item -- элемент, который можно положить в инвентарь
- Empty -- пустое поле

Класс Hero в данный момент имеет одного наследника Girl, но оставляет возможность для расширения -- добавления новых типов персонажей.

Класс Item имеет двух наследников: Wings и Quadrocopter, но также оставляет возможность добавлять новые предметы.
